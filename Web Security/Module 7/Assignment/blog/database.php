...
class Database {
    ...
    public static function checkLogin($username, $password): array {
        ...
        if (!isset($username) || !isset($password)) {
            return ['login_success'=>$login_success,'user_id'=>$user_id];
        }
        ...
        $pdo = self::getConnection();
        $sql = "SELECT id, password, salt FROM members WHERE username = ? LIMIT 1";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$username]);
        $user = $stmt->fetch();
        ...
        if ($user) {
            ...
            $login_success=$check_password === $user['password'];
            $user_id=$user['id'];
        }
        return ['login_success'=>$login_success,'user_id'=>$user_id];
    }
